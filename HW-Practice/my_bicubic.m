function [F1, C] = my_bicubic(F0,C0,C)
    ...
    %–аботаем с квадратом 4х4 изначальной сетки
    %интерполируема€ сетка, X0(i,j) - координата по x
    tic;
    X0 = C0{1};
    Y0 = C0{2};
    ...
    p=1; q=1;   %индексы интерполируемой точки на интерпол€ционной сетке
    
    %i,j - координаты верхней левой точки квадрата интерполируемого
    %квадрата на интерпол€ционной сетке
    
    %дл€ нахождени€ значени€ производных нужны боковые границы на начальной
    %сетке в 1 шаг.
    for i=2:1:size(X0,1)-2
        p=1;
        for j=2:1:size(Y0,2)-2            
            tic;
            %ищем начальную точку на интерпол€ционной сетке
            while(X1(p,p)<X0(i,j))
                p = p + 1;
            end
            while(Y1(q,q)<Y0(i,j))
            ...
            ders = zeros(1,2*4);
            temp=1;
            %производные по ’ дл€ узлов квадрата, а также дл€ узлов на 1
            %выше и ниже него
            %заполн€ем по строкам
            for m=(-1:1:2)
                for n=[0,1]
